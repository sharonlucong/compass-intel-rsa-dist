    ...
    WizardService = (function() {
      ...
      WizardService.prototype.setWizardPreConfig = function(currentAdapterName, clusterConfigData) {
        switch (currentAdapterName[0]) {
          ...
          case "ceph_openstack_icehouse":
            return this.getWizardPreConfig("openstack_ceph", clusterConfigData);
          case "ceph_firefly":
            return this.getWizardPreConfig("ceph_firefly", clusterConfigData);
        }
      };
      ...
      WizardService.prototype.setRsaMachinesToLocal = function(machines) {
        return this.wizardFactory.setRsaMachines(machines);
      };

      WizardService.prototype.svSelectionInit = function($scope) {
        var setRsaMachinesToLocal, wizardFactory;
        setRsaMachinesToLocal = this.setRsaMachinesToLocal;
        wizardFactory = this.wizardFactory;
        this.getRSABaseURL().success(function(data) {
          return $scope.rsaBaseURLs = data;
        });
        this.getExistingRsaMachines().success(function(data) {
          var machine, memory, rsaMachineContainer, _i, _j, _len, _len1, _ref, _results;
          rsaMachineContainer = wizardFactory.getRsaMachines();
          rsaMachineContainer.length = 0;
          _results = [];
          for (_i = 0, _len = data.length; _i < _len; _i++) {
            ...
            _results.push(rsaMachineContainer.push(machine));
          }
          return _results;
        });
        ...
        $scope.search = {};
        $scope.searchRSA = {};
        $scope.cluster = this.getClusterInfo();
        $scope.allservers = this.getAllMachinesHost();
        $scope.allAddedSwitches = [];
        $scope.allRSAMachines = this.getExistingRsaMachinesFromLocal();
        $scope.showDetail = false;
        $scope.rsaGetMachineStatus = "";
        return this.getDetailDisplayName($scope);
      };
      ...
      WizardService.prototype.svSelectonCommit = function($scope) {
        var addHostsAction, buildMachineObjectHelper, noRSASelection, noSelection, selectedServers, server, sv, wizardFactory, _i, _j, _k, _l, _len, _len1, _len2, _len3, _ref, _ref1, _ref2, _ref3;
        ...
        noRSASelection = true;
        _ref1 = $scope.allRSAMachines;
        for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
          sv = _ref1[_j];
          if (sv.check) {
            noRSASelection = false;
        ...
        if (noSelection && noRSASelection) {
          return this.wizardFactory.setCommitState({
            "name": "sv_selection",
            "state": "invalid",
            "message": "Please select at least one server"
          });
        } else if (!noSelection && !noRSASelection) {
          return this.wizardFactory.setCommitState({
            "name": "sv_selection",
            "state": "invalid",
            "message": "You must choose machines either 'Switch' or 'RSA'!"
          });
        } else {
          wizardFactory = this.wizardFactory;
          addHostsAction = {
          ...
          _ref3 = $scope.allRSAMachines;
          for (_l = 0, _len3 = _ref3.length; _l < _len3; _l++) {
            server = _ref3[_l];
            if (server.check) {
              addHostsAction.add_hosts.machines.push(buildMachineObjectHelper({
      ...
      WizardService.prototype.displayDataInTable = function($scope, data, tableName) {
      ...
      WizardService.prototype.triggerFindRsaMachine = function(id) {
        return this.dataService.triggerFindRsaMachine(id);
      };

      WizardService.prototype.getRSABaseURL = function() {
        return this.dataService.getRSABaseURL();
      };

      WizardService.prototype.getDetailDisplayName = function($scope) {
        return this.dataService.getDetailDisplayName().success(function(data) {
          return $scope.detailDisplayName = data;
        });
      };

      WizardService.prototype.getAllRsaMachinesPeriodically = function($scope, rsaBaseUrl) {
        var $filter, count, dataService, getRSAMachines, maxTry;
        dataService = this.dataService;
        $filter = this.$filter;
        maxTry = 10;
        count = 1;
        getRSAMachines = function() {
          return dataService.getFindRsaMachineStatus(rsaBaseUrl.id).success(function(data) {
            rsaBaseUrl.rsaGetMachineStatus = data.polling_status;
            if (data.polling_status === "under_monitoring") {
              return dataService.getAllRsaMachines(rsaBaseUrl.id).success(function(data) {
                ...
                for (_i = 0, _len = data.length; _i < _len; _i++) {
                  ...
                  $scope.allRSAMachines.push(machine);
                }
                $scope.allRSAMachines = $filter('unique')($scope.allRSAMachines, "mac");
                return console.log($scope.allRSAMachines);
              });
            } else {
              if (count < maxTry) {
                count++;
                return rsaBaseUrl.timeout = setTimeout(getRSAMachines, 2000);
              } else {
                return rsaBaseUrl.rsaGetMachineStatus = "error";
              }
            }
          });
        };
        clearTimeout(rsaBaseUrl.timeout);
        return getRSAMachines();
      };

      WizardService.prototype.getExistingRsaMachines = function() {
        return this.dataService.getExistingRsaMachines();
      };

      WizardService.prototype.getExistingRsaMachinesFromLocal = function() {
        return this.wizardFactory.getRsaMachines();
      };

      WizardService.prototype.addRsaManager = function(url) {
        return this.dataService.addRsaManager({
          manager_base_url: url
        });
      };

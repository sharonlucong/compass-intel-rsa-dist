define(['./baseService'], () -> 
    ...
    class DS
        ...
        getRSABaseURL: ()->
            return @$http.get(@settings.apiUrlBase + '/rsa-managers')

        getAllRsaMachines: (id)->
            return @$http.get(@settings.apiUrlBase + '/rsa-managers/' + id + '/machines/details')

        getDetailDisplayName: ()->
            return @$http.get(@settings.metadataUrlBase + '/rsa_machine_detail_map.json')

        getExistingRsaMachines: ()->
            return @$http.get(@settings.apiUrlBase + '/rsa-machines?show_details')

        triggerFindRsaMachine: (id)->
            return @$http.post(@settings.apiUrlBase + '/rsa-managers/' + id)

        getFindRsaMachineStatus: (id)->
            return @$http.get(@settings.apiUrlBase + '/rsa-managers/' + id)
        addRsaManager: (request)->
            return @$http.post(@settings.apiUrlBase + '/rsa-managers', angular.toJson(request))
        ...
        getPodRacks: (pod_id)->
            return @$http.get(@settings.apiUrlBase + '/rsa-managers/' + pod_id + '/racks')

        getOneRack: (rack_id)->
            return @$http.get(@settings.apiUrlBase + '/rsa-racks/' + rack_id)

        getRackDrawers: (rack_id) ->
            return @$http.get(@settings.apiUrlBase + '/rsa-racks/' + rack_id + '/drawers')

        getRackThermalZones: (rack_id) ->
            return @$http.get(@settings.apiUrlBase + '/rsa-racks/' + rack_id + '/thermal-zones')
        
        getRackPowerZones: (rack_id) ->
            return @$http.get(@settings.apiUrlBase + '/rsa-racks/' + rack_id + '/power-zones')

        getRackDrawerModules: (drawer_id) ->
            return @$http.get(@settings.apiUrlBase + '/rsa-drawers/' + drawer_id + '/modules')
        ...
        getRackPowerZonesSupplyUnits: (rack_id, zone_id) ->
            return @$http.get(@settings.apiUrlBase + '/rsa-racks/' + rack_id + '/power-zones/' + zone_id + '/supply-units')

        getRackThermalZonesFans: (rack_id, zone_id) ->
            return @$http.get(@settings.apiUrlBase + '/rsa-racks/' + rack_id + '/thermal-zones/' + zone_id + '/fans')
       
        getCpuUsage: (rack_id) ->
            return @$http.get(@settings.apiUrlBase + '/rsa-racks/' + rack_id + '/cpu-usage')

        getMemoryUsage: (rack_id)->
            return @$http.get(@settings.apiUrlBase + '/rsa-racks/' + rack_id + '/memory-usage')

        getStorageUsage: (rack_id) ->
            return @$http.get(@settings.apiUrlBase + '/rsa-racks/' + rack_id + '/storage-usage') 

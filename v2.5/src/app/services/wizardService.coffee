define(['./baseService'], ()-> 
    ...
    class WizardService
        ...
        wizardInit: ($scope, clusterId, clusterData, adaptersData, wizardStepsData, machinesHostsData, clusterConfigData) ->
            ...
            $scope.maxStep = 1
            $scope.pendingStep = 1
        setRsaMachinesToLocal: (machines)->
            @wizardFactory.setRsaMachines machines

        svSelectionInit: ($scope) ->
            setRsaMachinesToLocal = @setRsaMachinesToLocal
            wizardFactory = @wizardFactory
            @getRSABaseURL().success (data) ->
                $scope.rsaBaseURLs = data
            @getExistingRsaMachines().success (data) ->
                ...
                for machine in data
                    ...
                    rsaMachineContainer.push(machine)
            $scope.hideunselected = ''
            $scope.search = {}
            $scope.searchRSA = {}
            $scope.cluster = @getClusterInfo()
            $scope.allservers = @getAllMachinesHost()
            $scope.allAddedSwitches = []
            $scope.allRSAMachines = @getExistingRsaMachinesFromLocal()
            $scope.showDetail = false
            $scope.rsaGetMachineStatus = ""
            @getDetailDisplayName($scope)
        ...
        svSelectonCommit: ($scope) ->
            ...
            noRSASelection = true
            (noRSASelection = false; selectedServers.push({"machine_id": sv.id+""})) for sv in $scope.allRSAMachines when sv.check
            ...
            if noSelection and noRSASelection
            ...
            else if !noSelection and !noRSASelection
                @wizardFactory.setCommitState(
                    "name": "sv_selection"
                    "state": "invalid"
                ...
                addHostsAction.add_hosts.machines.push(buildMachineObjectHelper({"machine_id": server.id})) for server in $scope.allRSAMachines when server.check
        ...
        triggerFindRsaMachine: (id)->
            @dataService.triggerFindRsaMachine(id)

        getRSABaseURL: ()->
            @dataService.getRSABaseURL()

        # getAllRsaMachines: (resource_id, dataService)->
        #     dataService.getAllRsaMachines(resource_id)

        getDetailDisplayName: ($scope)->
            @dataService.getDetailDisplayName().success (data)->
                $scope.detailDisplayName = data

        getAllRsaMachinesPeriodically: ($scope, rsaBaseUrl)->
            # getAllRsaMachines = @getAllRsaMachines
            dataService = @dataService
            $filter = @$filter
            maxTry = 10
            count = 1
            getRSAMachines = () ->
                dataService.getFindRsaMachineStatus(rsaBaseUrl.id).success (data) ->
                    rsaBaseUrl.rsaGetMachineStatus = data.polling_status
                    if data.polling_status is "under_monitoring"
                        dataService.getAllRsaMachines(rsaBaseUrl.id).success (data)->
                            for machine in data
                                machine.memorySize = 0
                                for memory in machine.machine_memories
                                    machine.memorySize += memory.memory_module_capacity
                            ...
                            console.log($scope.allRSAMachines)
                    else
                        if count < maxTry
                            count++
                            rsaBaseUrl.timeout = setTimeout(getRSAMachines, 2000)
                        else
                            rsaBaseUrl.rsaGetMachineStatus = "error"

            clearTimeout(rsaBaseUrl.timeout)
            getRSAMachines()
        getExistingRsaMachines: ()->
            @dataService.getExistingRsaMachines()
        getExistingRsaMachinesFromLocal: ()->
            @wizardFactory.getRsaMachines()
        addRsaManager: (url)->
            @dataService.addRsaManager({manager_base_url: url})
